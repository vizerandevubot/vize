...
def build_minute_keyboard():
    rows = [
        [InlineKeyboardButton(f":{m:02d}", callback_data=f"min|{m}") for m in (0, 15, 30, 45)],
...
def button_router(update: Update, context: CallbackContext):
    ...
    if data == "menu":
        ...
        query.answer()
        query.edit_message_text("Ana Menu:", reply_markup=build_main_menu_keyboard())
        return
    ...
    if data.startswith("cal|"):
        ...
        if direction == "prev":
            ...
            if month == 13:
                ...
                year += 1
        query.answer()
        query.edit_message_reply_markup(reply_markup=build_calendar_keyboard(year, month))
        return
    ...
    if data.startswith("day|"):
        ...
        query.answer()
        query.edit_message_text("Simdi saati secin:", reply_markup=build_hour_keyboard())
        return
    ...
    if data.startswith("hour|"):
        ...
        query.answer()
        query.edit_message_text("Simdi dakikayi secin:", reply_markup=build_minute_keyboard())
        return
    ...
    if data.startswith("alerttoggle|"):
        ...
        if minutes in alerts:
            ...
            alerts.add(minutes)
        query.answer()
        query.edit_message_reply_markup(reply_markup=build_alert_keyboard(chat_id))
        return
    ...
    if data == "list":
        query.answer()
        query.edit_message_text(build_reminder_list_text(chat_id), reply_markup=build_main_menu_keyboard())
        return
